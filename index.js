...
app.post('/films', async (req, res) => { // Endpoint to create a new Komik
    const data = req.body; // Get data from request body
    try {
        const film = await db.Film.create(data); // Create a new Komik record
...
app.get('/films', async (req, res) => { // Endpoint to get all Komiks
    try {
        const film = await db.Film.findAll(); // Retrieve all Komik records
...
app.put('/films/:id', async (req, res) => { // Endpoint to update a Komik by ID
    const id = req.params.id; // Get ID from request parameters
    const data = req.body; // Get updated data from request body

    try {
        const film = await db.Film.findByPk(id); // Find Komik by primary key
        if (!film) { // If Komik not found
            return res.status(404).send({ message: 'Film not found' }); // Send 404 response
        }

        await film.update(data); // Update the Komik record
...
app.delete('/films/:id', async (req, res) => { // Endpoint to delete a Komik by ID
    const id = req.params.id; // Get ID from request parameters
    try {
        const film = await db.Film.findByPk(id); // Find Komik by primary key
        if (!film) { // If Komik not found
        ...
        await film.destroy(); // Delete the Komik record
